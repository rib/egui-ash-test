...
use std::{
    ffi::CString, mem::ManuallyDrop, sync::{Arc, Mutex}
};
...
impl RendererInner {
    ...
    fn load_model_and_create_vertex_buffer(
        ...
        let vertices = {
            let model_bytes = include_bytes!("assets/suzanne.obj");
            let mut model_slice: &[u8] = model_bytes.as_slice();
            let model_obj = tobj::load_obj_buf(
                &mut model_slice,
                &tobj::LoadOptions {
                    single_index: true,
                    triangulate: true,
